...
GraphicObject::GraphicObject() {
	...
	setRotateAngle(glm::vec3(0.0, 0.0, 0.0));
	setColor(glm::vec3(1.0f, 1.0f, 1.0f));
	setSize(glm::vec3(1.0f, 1.0f, 1.0f));
}
GraphicObject::GraphicObject(glm::vec3 pos, glm::vec3 rotateAng, glm::vec3 color, glm::vec3 size) {
	initGraphicObjectData();
	setPosition(pos);
	setRotateAngle(rotateAng);
	setColor(color);
	setSize(size);
}
GraphicObject::~GraphicObject() {
...
void GraphicObject::setRotateAngle(glm::vec3 rotateAng) {
	angle = rotateAng;
	...
	modelMatrix = glm::rotate(modelMatrix, glm::radians(angle.z), glm::vec3(0.0f, 1.0f, 1.0f));
}
glm::vec3 GraphicObject::getRotateAngle() {
	return angle;
}
